...
// takes the exact same {model, messages, max_tokens, temperature} request
// shape as the local llama-server call below — only the URL, auth header,
// and model name differ. That's what makes switching providers a matter of
// picking which fetch to make rather than reshaping every caller.
...
function setLlmProvider(provider) {
  ...
  return { ok: true, message: normalized === 'gemini'
    ? `Switched to Gemini (${GEMINI_MODEL}).`
    : `Switched to local model (${MODEL}). If llama-server isn't already running, start it (restart Aigentik, or run ./start.sh).` };
}
...
async function chat(messages, maxTokens = MAX_TOKENS) {
  return getLlmProvider() === 'gemini' ? chatGemini(messages, maxTokens) : chatLocal(messages, maxTokens);
}
