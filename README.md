...
Pass `--skip-llama` and/or `--skip-model` if you already have a `llama-server` binary or GGUF model elsewhere and want to point `config.json` at those manually instead (see the [Configuration reference](#configuration-reference)).
...
Edit `config.json` — see the [full reference below](#configuration-reference) for every field. At minimum you need to fill in `gmail.email`, `gmail.app_password`, `owner.admin_number*`, `owner.admin_email`, and the `llama.*` paths to match where you built llama.cpp and put the model in steps 3–4.
